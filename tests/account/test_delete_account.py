...
async def test_delete_account_success(client: AsyncClient, access_token_manager: str):
    # Given
    headers = {"Authorization": f"Bearer {access_token_manager}"}
    acc_id = 1

    # When
    response = await client.delete(f"/accounts/{acc_id}", headers=headers)

    # Then
    assert response.status_code == status.HTTP_204_NO_CONTENT


async def test_delete_account_not_authenticated_fail(client: AsyncClient):
    # Given
    acc_id = 1

    # When
    response = await client.delete(f"/accounts/{acc_id}", headers={})

    # Then
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


async def test_delete_account_not_found_success(client: AsyncClient, access_token_manager: str):
    # Given
    headers = {"Authorization": f"Bearer {access_token_manager}"}
    acc_id = 4
