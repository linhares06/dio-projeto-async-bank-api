...
async def test_read_user_success(client: AsyncClient, access_token_manager: str):
    # Given
...
async def test_read_user_not_authenticated_fail(client: AsyncClient):
    # Given
    acc_id = 1
...
async def test_read_user_not_found_fail(client: AsyncClient, access_token_manager: str):
    # Given
    headers = {"Authorization": f"Bearer {access_token_manager}"}
    acc_id = 7
